...
let users: any[] = userFromStorage ? JSON.parse(userFromStorage) : [];
...
export class FakeBackendInterceptor implements HttpInterceptor {
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        function handleRoute() {
            switch (true) {
                case url.endsWith('/reset') && method === 'POST':
                    return resetPassword();
                case url.endsWith('/account/signin') && method === 'POST':
                    return authenticate();
                case url.endsWith('/account/signup') && method === 'POST':
                    return register();
                case url.endsWith('/account/extendToken') && method === 'POST':
                    return extendToken();
